...
func (scan *Scanner) Slice() string {
  slice := scan.text[scan.markIdx:scan.Idx]
...
func (scan *Scanner) SliceIncl() string {
  slice := scan.text[scan.markIdx:scan.Idx+1]
...
func ForEach(text string, fn func(RuneSpan) bool) {
  scan := NewScanner(text)
  for { if span := scan.PopSpan(); span.Rune == ScanEOF || !fn(span) { return } }
}